...
const FaqSection = ({ faqs }) => {
  ...
  return (
    <section className="mt-5 flex flex-col mx-10 lg:mx-35 gap-10 text-white">
      <header className="flex flex-col gap-4 text-left">
        <h2 className="text-3xl lg:text-5xl font-bold md:text-4xl">
          ...
          >
            <button
              className={`flex w-full items-center justify-between gap-4 px-7 py-6 text-left text-base font-semibold text-white transition-colors duration-300 md:text-lg ${
                openFaqIndex === index ? "text-[#8b5cf6]" : "hover:text-[#8b5cf6]"
              }`}
              onClick={() => toggleFaq(index)}
              aria-expanded={openFaqIndex === index}
            >
              <span className="flex-1 text-left leading-snug text-[15px]">
                {faq.question}
              </span>
              <span
